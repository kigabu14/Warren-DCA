...
Enhanced with additional stock metrics: open_price, close_price, dividend_amount, roe, eps.
"""
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.executescript("""
                ...
                CREATE TABLE IF NOT EXISTS analyses (
                    ...
                    -- Enhanced stock metrics (original fields)
                    dividend_yield REAL,
                    ...
                    -- NEW: Additional stock metrics as requested
                    open_price REAL,
                    close_price REAL,
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("""
                INSERT INTO analyses (
                    ...
                    open_price, close_price, dividend_amount, roe, eps
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                stock_id,
                ...
                # NEW fields
                analysis_data.get('open_price'),
                analysis_data.get('close_price'),
                analysis_data.get('dividend_amount'),
                analysis_data.get('roe'),
                analysis_data.get('eps')
            ))
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.execute("""
                SELECT 
                    ...
                    a.last_close, a.open_price, a.close_price, a.dividend_amount,
                    a.roe, a.eps
                FROM analyses a
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.execute("""
                SELECT 
                    ...
                    AVG(a.eps) as avg_eps,
                    AVG(a.dividend_amount) as avg_dividend_amount
                FROM stocks s
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.execute("""
                SELECT 
                    ...
                    a.open_price, a.close_price, a.dividend_amount, a.roe, a.eps
                FROM analyses a
                JOIN stocks s ON a.stock_id = s.id
                WHERE s.ticker = ?
                ORDER BY a.analysis_date DESC
